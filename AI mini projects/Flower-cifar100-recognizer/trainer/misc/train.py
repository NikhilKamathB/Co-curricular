...
class Train:
    ...
    def __init__(self, model, train_loader, optimizer, criterion, scheduler, epochs, device, save_path_dir=None, verbose=True, verbose_step=50, run=None, misc=None, wandb_needed=False):
        self.model = model
    ...
    def plot_loss_curve_wandb(self):
            ...
            for step, (images, labels) in enumerate(self.train_loader): 
                ...
                if self.verbose:
                    if (step+1) % self.verbose_step == 0:
                        ...
                        if self.wandb:
                            wandb.log({"Training Step Loss": step_running_loss/self.verbose_step}, step=step_size)
                        step_running_loss = 0   
                        start_step_time = time.time()
