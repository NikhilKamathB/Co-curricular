...
def interaction_model(p_player:tuple, v_player:tuple, p_ego:tuple, v_ego:tuple) -> tuple:
    ...
    next_p_player = (p_player[0]+(x_hat*t*1e-3), p_player[1]+(y_hat*t*1e-3))
    if next_p_player[0] < 0 or next_p_player[1] < 0 or next_p_player[0] > world[0] or next_p_player[1] > world[1]:
        next_p_player = (min(max(0, next_p_player[0]), world[0]), min(max(0, next_p_player[1]), world[1]))
        v_player_magnitude = np.linalg.norm((next_p_player[0]-p_player[0], next_p_player[1]-p_player[1]))
        h_player = np.arctan2(p_player[1]-next_p_player[1], p_player[0]-next_p_player[0])
        x_hat, y_hat = v_player_magnitude*np.cos(h_player), v_player_magnitude*np.sin(h_player)
        d_player_ego = distance(next_p_player, next_p_ego)
    return (d_player_ego, next_p_player, (x_hat, y_hat))
